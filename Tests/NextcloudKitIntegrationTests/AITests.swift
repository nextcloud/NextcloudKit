...
final class AITests: BaseIntegrationXCTestCase {
    func testTest() {
...
//               let serverUrl = "\(TestConstants.server)/remote.php/dav/files/\(TestConstants.username)"
               let serverUrl = "https://daily.ltd2.nextcloud.com"
//               let serverUrlFileName = "\(serverUrl)/\(folderName)"
               let account = "milen.pivchev@nextcloud.com https://daily.ltd2.nextcloud.com"

        NextcloudKit.shared.textProcessingGetTypes(account: account) { account, types, responseData, error in
            print("YES")
            expectation.fulfill()
        }
