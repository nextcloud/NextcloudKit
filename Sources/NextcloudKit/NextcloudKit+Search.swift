...
public extension NextcloudKit {
    ...
    func unifiedSearch(term: String,
        ...
        let requestUnifiedSearch = nkSession.sessionData.request(url, method: .get, parameters: nil, encoding: URLEncoding.default, headers: headers, interceptor: Interceptor.shared).validate(statusCode: 200..<300).onURLSessionTaskCreation { task in
    ...
    func searchProvider(_ id: String,
        ...
        let requestSearchProvider = nkSession.sessionData.request(urlRequest, interceptor: Interceptor.shared).validate(statusCode: 200..<300).onURLSessionTaskCreation { task in
            task.taskDescription = options.taskDescription
            taskHandler(task)
        }.responseData(queue: self.nkCommonInstance.backgroundQueue) { response in
            if self.nkCommonInstance.levelLog > 0 {
